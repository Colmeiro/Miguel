...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        /**
         * PROPIO.
         * Añadido para poder desencriptar key propia mediante helper crypt propio
         */
    ...
    public function response($data = NULL, $http_code = NULL, $continue = FALSE)
    {
		...
		if(!$isProfilingEnabled){
        	...
        	elseif ($data !== NULL)
        	{
                ...
                if (method_exists(get_class(new Format), 'to_' . $this->response->format))
            	{
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            /**
             * PROPIO.
             * Comprobamos si en la configuración de rest está establecido usar key propio con el helper crypt_helper creado o bien se usa la generación de clave aleatoria tal y como viene en el REST_Controller original.
             */
            if($this->config->item('is_key_by_crypt')) {
                return $this->_check_key_by_crypt($key);
            }
            ...
            return TRUE;
        } else if($this->config->item('key_nokey_both_allowed')) {
            /**
             * PROPIO.
             * 
             * Crear variable en el config de rest para permitir casos de conexión sin api key.
             * Cuando esté a true dicha variable, se comprueba si viene la key en la cabecera. Si viene, se sigue el proceso de las api keys. Si no viene la key, se ignora el proceso de las api keys, permitiendo la solicitud de la api igualmente.
             * Para casos en que aún no tengan la app actualizada los clientes pero que podamos probarla en producción igualmente.
             */
            return TRUE;
        }
    ...
    /**
     * PROPIO.
     * Comprobar key con método crypt y clave propios.
     *
     * @access protected
     * @return bool
     */
    protected function _check_key_by_crypt($key)
    {
        $this->user_id = getIdFromToken($key);
        ...
        $this->db->where($this->config->item('user_id_field'),$this->user_id);
        $this->db->from($this->config->item('user_table'));
     ...
     * PROPIO.
     */
    protected function _generateNewKey($dataToEncrypt='')
    {
        /**
         * Comprobamos si en la configuración de rest está establecido usar key propio con el helper crypt_helper creado o bien se usa la generación de clave aleatoria tal y como viene en el REST_Controller original.
         */
        ...
        /**
         * Si la configuración es por defecto, pasamos a generar una clave aleatoria.
         * 
         * Generar Key desde el propio api. Más seguro, para no desvelar la key inicial que nos sirve para poder generar una nueva key.
         */

        $initalKey = $this->_getInitialKey();
        if(empty($initalKey->key)) {
            return FALSE;
        }

        $data = array("level" => "20");

        $ch = curl_init(base_url() . '/api/key');

        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
        curl_setopt($ch, CURLOPT_HTTPHEADER, array(
            'X-API-KEY: ' . $initalKey->key
        ));
     ...
     * PROPIO.
     */
    private function _getInitialKey()
    {
        return $this->rest->db
            ->where('level', 10)
