...
class Authentication {
	...
	function __construct()
	{
		...
		$this->name_field = $authentication_config['name_field'];
	...
	public function login($username, $password)
	{
		...
		// Select user details
		$user = $this->ci->db
			->select($this->user_table . "." . $this->identifier_field.' as identifier, '.$this->username_field.' as username,CONCAT('.$this->user_table . '.'.$this->name_field.', " ", usuario.apellidos) as name, '.$this->password_field.' as password, '.$this->user_table . '.nombre, rol.nombre as rol')
			->join("rol", $this->user_table . ".rol_id = rol.rol_id", "left")
			->where($this->username_field, $username)
			->where($this->user_table . '.activo',1)
			->get($this->user_table);
	 ...
	 * Login By Id
	 * 
	 * Variación para poder hacer login con id de usuario
	 *
	 * @access	public
	 * @param	string [$identifier] The id of the user to authenticate
	 * @return	boolean Either TRUE or FALSE depending upon successful login
	 */
	public function login_by_id($identifier)
	{
		// Select user details
		$user = $this->ci->db
			->select($this->identifier_field.' as identifier, '.$this->username_field.' as username, CONCAT('.$this->name_field.', " ", usuario.apellidos) as name, '.$this->password_field.' as password')
			->where($this->identifier_field, $identifier)
			// ->where('activo',1)
			->get($this->user_table);

		// Ensure there is a user with that id
		if ($user->num_rows() == 0)
		{
			// There is no user with that id, but we won't tell the user that
			return FALSE;
		}
	 ...
	 * Set User Data
	 * 
	 * Meter los datos del usuario que se logea en la sesión. 
	 *
	 * @access	public
	 * @param	array [$user_details] The user details of the user
	 * @return	boolean Either TRUE or FALSE depending upon successful login
	 */
	public function set_user_data($user_details)
	{
		// Set the userdata for the current user
	...
	public function changepassword($user_identifier,$password)
	{
	...
	public function read($key)
	{
		...
		switch ($key)
		{
			...
			case 'name': {
				...
				// Return name
				return (string) $this->ci->session->userdata('name');
