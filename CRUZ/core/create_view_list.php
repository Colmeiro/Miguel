...
$string .= '            <div class="heading-elements search-pep">'."\n";
$string .= '                <ul class="list-inline mb-0">'."\n";
$string .= '                    <li class="search-li">'."\n";
$string .= '                        <?/* <form action="<?php echo site_url(\''.$c_url.'/view\'); ?>" method="post"> */?>'."\n";
$string .= '                            <fieldset class="has-icon-left position-relative">'."\n";
$string .= '                                <input type="text" id="buscartabla" class="form-control single-daterange" placeholder="<?php echo $total_rows ?> Resultados" name="q" value="<?= $q ?>">'."\n";
$string .= '                                <div class="form-control-position">'."\n";
$string .= '                                   <i class="bx bx-search font-medium-1"></i>'."\n";
$string .= '                                </div>'."\n";
$string .= '                                <button class="btn btn-light search-btn-pep rounded" type="button">'."\n";
$string .= '                                   <span><i class="bx bx-right-arrow-alt"></i></span>'."\n";
$string .= '                                </button>'."\n";
$string .= '                            </fieldset>'."\n";
$string .= '                        <?/* </form> */?>'."\n";
$string .= '                    </li>'."\n";
...
if ($action_add == '1') {
    $string .= '                    <li class="ml-2"><?php echo anchor(site_url(\''.$c_url.'/create\'), \'Añadir <i class="bx bx-plus"></i>\', \'class="btn btn-primary add-btn"\'); ?></li>'."\n";
}
...
$string .= '            <table id="table-extended-dt" class="dt-' . $c_url_orig . ' table mb-0">'."\n";
$string .= '                <thead class="bg-pep">'."\n";
$string .= '                    <tr>'."\n";
...
if(isset($showkey)){
    ...
    $string .= '                        <th class="sorting w-80">'."\n";
    $string .= '                            <a>' . $nombrelabelkey . '</a>'."\n";
    $string .= '                        </th>'."\n";
}
...
foreach ($non_pk_list as $row) {
    if(in_array($row['column_name'],$list_campos)){
        $string .= "\n\t\t<th class=\"sorting \">
        <a>" . label($labels[$row['column_name']]) . "</a>
        </th>\n";
    }
}
if($posicion_botones=='derecha'){
    $string .= '                        <th class="text-center d-none d-sm-table-cell"><span>Acciones</span></th>'."\n";
    $string .= "                        <th class=\"d-table-cell d-md-none\"></th>\n";
}
...
$string .= "<td class=\"a-center \">
  ...
  $botones = "                            <!-- Acciones Desktop -->\n";
   $botones .= "                            <td class=\"text-center btn-acciones d-none d-md-table-cell\">\n";
if($formato_botones=='iconos'){
    $botones .= "                                <a href=\"<?=site_url('".$c_url."/read/'.\$row->". $pk . ") ?>\" class=\"btn btn-xs btn-icon-only btn-info btn-table\"><i class=\"bx bx-search\"></i></a>\n";
    if($action_edit == 1){
        $botones .= "                                <a href=\"<?=site_url('".$c_url."/update/'.\$row->". $pk . ") ?>\" class=\"btn btn-icon-only btn-xs btn-success btn-table\"><i class=\"bx bx-edit\"></i></a>\n";
    }
    if($action_del == 1){
        $botones .= "                                <a href=\"<?=site_url('".$c_url."/delete/'.\$row->". $pk . ") ?>\" onclick=\"javascript: return confirm('Seguro que deseas eliminar este ".$objeto."?')\" class=\"btn btn-xs btn-icon-only btn-danger btn-table\"><i class=\"bx bx-trash\"></i></a>\n";
    }
}
else{
    ...
    $botones .= "                            <!-- Acciones Mobile -->\n";
   $botones .= "                            <td class=\"text-center btn-acciones d-table-cell d-md-none\">\n";
   $botones .= "                            <div class=\"dropdown\">\n";
   $botones .= "                                <span class=\"bx bx-dots-vertical-rounded font-medium-3 dropdown-toggle nav-hide-arrow cursor-pointer\" data-toggle=\"dropdown\" aria-haspopup=\"true\" aria-expanded=\"false\" role=\"menu\"></span>\n";
   $botones .= "                                <div class=\"dropdown-menu dropdown-menu-right\">\n";

if($formato_botones=='iconos'){
    $botones .= "                                   <a class=\"dropdown-item\" href=\"<?=site_url('".$c_url."/read/'.\$row->". $pk . ") ?>\"><i class=\"bx bx-show-alt mr-1\"></i> Ver</a>\n";
    if($action_edit == 1){
        $botones .= "                                   <a class=\"dropdown-item\" href=\"<?=site_url('".$c_url."/update/'.\$row->". $pk . ") ?>\"><i class=\"bx bx-edit-alt mr-1\"></i> Editar</a>\n";
    }
    if($action_del == 1){
        $botones .= "                                   <a class=\"dropdown-item\" href=\"<?=site_url('".$c_url."/delete/'.\$row->". $pk . ") ?>\" onclick=\"javascript: return confirm('Seguro que deseas eliminar este ".$objeto."?')\"><i class=\"bx bx-trash mr-1\"></i> Eliminar</a>\n";
    }
}
else{
    $botones .= "                                   <a href=\"<?=site_url('".$c_url."/read/'.\$row->". $pk . ") ?>\" class=\"btn-text-only mb-1 mb-md-0\">Ver </a>\n";
    ...
    foreach ($non_pk_list as $row) {
        if(in_array($row['column_name'],$list_campos)){
            ...
            $string .= "                            <td class=\"".$align."\" data-order=\"<?= \$row->". $row['column_name'] . " ?>\" data-search=\"<?= \$row->". $row['column_name'] . " ?>\">";
