...
if($template=='gentelella'){
    ...
    $string .= '<div class="row"><div class="col-md-12 col-sm-12 col-xs-12"><div class="x_panel"><div class="x_content">';
}
// $string .= "        <h2 style=\"margin-top:0px\">".ucfirst($titulo)."</h2>";
        // if($template=='adminlte'){
        //     $string .= '</div><div class="box-header">';
        // }
        $string .= "<div class=\"row color-fondo\">
            <div class=\"col-md-3\">";
            if ($action_add == '1') {
...
if($posicion_botones=='izquierda'){
    $string .= '<th class="column-title">Acciones</th>';
}
$non_pk_list = array();
...
if($formato_botones=='iconos'){
    $botones .= "\n\t <a href=\"<?=site_url('".$c_url."/read/'.\$row->". $pk . ") ?>\" class=\"btn btn-xs btn-icon-only btn-info btn-table\"><i class=\"fa fa-search\"></i></a>";
    if($action_edit == 1){
        $botones .= "\n\t <a href=\"<?=site_url('".$c_url."/update/'.\$row->". $pk . ") ?>\" class=\"btn btn-icon-only btn-xs btn-info btn-table\"><i class=\"fa fa-edit\"></i></a>  ";
    }
    if($action_del == 1){
        $botones .= "\n\t <a href=\"<?=site_url('".$c_url."/delete/'.\$row->". $pk . ") ?>\" onclick=\"javascript: return confirm('Seguro que deseas eliminar este ".$objeto."?')\" class=\"btn btn-xs btn-icon-only btn-danger btn-table\"><i class=\"fa fa-trash\"></i></a>";
    }
}
else{
    $botones .= "\n\t <a href=\"<?=site_url('".$c_url."/read/'.\$row->". $pk . ") ?>\" class=\"btn-text-only\">Ver </a>";
    if($action_edit == 1){
        $botones .= "\n\t <a href=\"<?=site_url('".$c_url."/update/'.\$row->". $pk . ") ?>\" class=\"btn-text-only\">Editar</a>  ";
    }
    if($action_del == 1){
        $botones .= "\n\t <a href=\"<?=site_url('".$c_url."/delete/'.\$row->". $pk . ") ?>\" onclick=\"javascript: return confirm('Seguro que deseas eliminar este ".$objeto."?')\" class=\"btn-text-only\">Eliminar</a>";
    }
}
    $botones .= "\n\t </td>";
...
$string .= "\n\t    </div>
            <div class=\"col-md-6 text-right\">
                ...
                if($template=='gentelella'){
                    $string .= '<div class="dataTables_paginate paging_simple_numbers">';
                }
                $string .= "    <?php echo \$pagination ?>";
                if($template=='gentelella'){
                    $string .= '</div>';
                }
                $string .= "</div>
